...
void CCamera::RegenerateViewMatrix() {
	...
	//카메라의 z-축 벡터를 정규화한다. 
	m_xmf3Look = Vector3::Normalize(m_xmf3Look); //카메라의 z-축과 y-축에 수직인 벡터를 x-축으로 설정한다. 
...
/*
	...
	스페이스-쉽 카메라는 플레이어 객체에 물리적으로 고정된다.
	그러므로 카메라 이동과 회전은 플레이어의 이동과 회전과 일치하게 된다.
...
/*
	...
	플레이어의 로컬 x-축을 기준으로 플레이어를 회전하면 카메라가 같은 회전을 하게 되므로
	카메라의 로컬 x-, y-, z-축이 바뀐다.
	플레이어의 중심과 카메라의 중심 위치가 다르면 플레이어의 자전에 카메라는 공전을 하게된다.
...
void CSpaceShipCamera::Rotate(float x, float y, float z) {
	...
	if (m_pPlayer && (x != 0.0f)) {
		...
		float3 = m_pPlayer->GetPosition();
		m_xmf3Position = Vector3::Subtract(m_xmf3Position, float3);
...
/*
	...
	1인칭 카메라는 사람의 눈을 의미한다.
	1인칭 카메라의 y-축과 z-축의 회전은 플레이어를 기준으로 회전을 하지만
	x-축으로는 카메라를 기준으로 회전을 할 수 있다.
...
/*
	...
	1인칭 카메라의 x- 축 회전은 카메라의 로컬 x-축으로 회전하는 것이다.
	1인칭 카메라의 y- 축 회전은 플레이어의 로컬 y-축으로 회전하는 것이다.
	1인칭 카메라의 z- 축 회전은 플레이어의 로컬 z-축으로 회전하는 것이다.

	1인칭 카메라의 x- 축 회전은 고개를 위/아래로 끄덕이는 것
	1인칭 카메라의 y- 축 회전은 몸을 회전 시키는 것
	1인칭 카메라의 z- 축 회전은 플레이어의 로컬 z-축을 기준으로 회전하는 것

	게임에서 이러한 회전을 Lean이라 한다.
	플레이어의 몸통은 벽에 기대어 감추고 고개를 살짝 내미는 동작에 해당

	실제 몸통이나 고개의 회전은 일어나지 않는다.
...
void CFirstPersonCamera::Rotate(float x, float y, float z) {
	...
	if (m_pPlayer && (z != 0.0f)) {
		...
		XMFLOAT3 float3 = m_pPlayer->GetPosition();
		m_xmf3Position = Vector3::Subtract(m_xmf3Position, float3);
...
/*
	...
	3인칭 카메라는 제 3자의 눈으로 플레이어를 바라보는 것을 의미한다.
	보통 플레이어에서 일정한 거리를 두고 플레이어의 일정한 지점을 바라보도록 한다.
	3인칭 카메라의 움직임은 플레이어와 카메라를 막대기로 연결하여 고정한 것으로 상상하면 이해하기 슆다.

	플레이어를 움직이면 카메라는 자동으로 따라서 움직인다.
...
/*
	...
	3인칭 카메라의 회전은 플레이어를 회전하는 것이다.
	플레이어의 회전은 플레이어의 로컬 y-축을 중심으로 하는 회전만 가능하다.

	플레이어가 회전하면 카메라는 플레이어의 y-축을 중심으로 회전 , 공전을 한다음
	플레이어를 바라 봐야한다.
...
/*
	...
	3인칭 카메라를 회전시키면 카메라의 위치와 방향이 바뀌게 된다.
	그리고 카메라가 플레이어를 바라보도록 해야 하므로
	새로운 카메라의 위치벡터를 사용하여 카메라의 방향 벡터를 다시 지정한다.
...
void CThirdPersonCamera::SetLookAt(XMFLOAT3& xmf3LookAt) {
	...
	XMFLOAT3 float3 = m_pPlayer->GetUpVector();
	XMFLOAT4X4 mtxLookAt = Matrix4x4::LookAtLH(m_xmf3Position, xmf3LookAt, float3);

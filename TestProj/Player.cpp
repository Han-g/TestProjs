...
void CPlayer::Move(const XMFLOAT3& xmf3Shift, bool bUpdateVelocity)
{
	...
	else
	{
		...
		XMFLOAT3 float3 = xmf3Shift;
		if (m_pCamera) { m_pCamera->Move(float3); }
	}
}
...
void CPlayer::Rotate(float x, float y, float z)
{
	...
	if ((nCameraMode == FIRST_PERSON_CAMERA) || (nCameraMode == THIRD_PERSON_CAMERA))
	{
		...
		+89도 보다 크거나 -89도 보다 작으면 m_fPitch가 +89도 또는 -89도가 되도록
		회전각도(x)를 수정한다.*/
		if (x != 0.0f)
		{
			m_fPitch += x;
		...
		if (z != 0.0f)
		{
			...
			회전 각도는 -20.0~+20.0도 사이로 제한된다. z는 현재의 m_fRoll에서 실제 회전하는
			각도이므로 z만큼 회전한 다음 m_fRoll이 +20도 보다 크거나 -20도보다
		...
		/*플레이어를 회전한다. 1인칭 카메라 또는 3인칭 카메라에서 플레이어의 회전은
		로컬 y-축에서만 일어난다. 플레이어의 로컬 y-축(Up 벡터)을 기준으로 로컬
		z-축(Look 벡터)와 로컬 x-축(Right 벡터)을 회전시킨다. 기본적으로 Up 벡터를 기준으로
		회전하는 것은 플레이어가 똑바로 서있는 것을 가정한다는 의미이다.*/
		if (y != 0.0f)
		{
...
void CPlayer::Update(float fTimeElapsed)
{
	...
	플레이어의 새로운 위치가 유효한 위치가 아닐 수도 있고 또는 플레이어의 충돌 검사 등을 
	수행할 필요가 있다. 이러한 상황에서 플레이어의 위치를 유효한 위치로 다시 변경할 수 있다.*/
	...
	속도 벡터의 반대 방향 벡터를 구하고 단위 벡터로 만든다. 마찰 계수를 시간에 비례하도록 하여 
	마찰력을 구한다. 단위 벡터에 마찰력을 곱하여 감속 벡터를 구한다. 
...
CCamera* CPlayer::OnChangeCamera(DWORD nNewCameraMode, DWORD nCurrentCameraMode)
{
	...
	{
		XMFLOAT3 NomalizeRight = XMFLOAT3(m_xmf3Right.x, 0.0f, m_xmf3Right.z);
		m_xmf3Right = Vector3::Normalize(NomalizeRight);
		XMFLOAT3 NomalizeUp = XMFLOAT3(0.0f, 1.0f, 0.0f);
		m_xmf3Up = Vector3::Normalize(NomalizeUp);
		XMFLOAT3 NomalizeLook = XMFLOAT3(m_xmf3Look.x, 0.0f, m_xmf3Look.z);
		m_xmf3Look = Vector3::Normalize(NomalizeLook);
		
		m_fPitch = 0.0f;
		m_fRoll = 0.0f;
		/*Look 벡터와 월드좌표계의 z-축(0, 0, 1)이 이루는 각도(내적=cos)를 계산하여 
		y-축의 회전 각도 m_fYaw로 설정한다.*/
		XMFLOAT3 float3 = XMFLOAT3(0.0f, 0.0f, 1.0f);
		m_fYaw = Vector3::Angle(float3, m_xmf3Look);
	...
	* pd3dCommandList, ID3D12RootSignature* pd3dGraphicsRootSignature)
{
	...
	//플레이어의 위치를 설정한다.
	XMFLOAT3 float3 = XMFLOAT3(0.0f, 0.0f, -50.0f);
	SetPosition(float3);
...
왜냐하면 비행기 모델 메쉬는 다음 그림과 같이 y-축 방향이 비행기의 앞쪽이 되도록 모델링이 
되었기 때문이다. 그리고 이 메쉬를 카메라의 z- 축 방향으로 향하도록 그릴 것이기 때문이다.*/
...
CCamera *CAirplanePlayer::ChangeCamera(DWORD nNewCameraMode, float fTimeElapsed)
{
	...
	//플레이어를 시간의 경과에 따라 갱신(위치와 방향을 변경: 속도, 마찰력, 중력 등을 처리)한다. Update(fTimeElapsed);
	return(m_pCamera);
}
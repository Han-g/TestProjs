...
CPlayer::CPlayer() {
	...
	p_info.collision = collision_check;
	p_info.Position = GetPosition();
}
...
void CPlayer::Update(float fTimeElapsed) {
	...
	*/
	...
	if (collision_check) { Move(m_xmf3Velocity, false); }
...
CCamera* CPlayer::OnChangeCamera(DWORD nNewCameraMode, DWORD nCurrentCameraMode) {
	...
	*/
	...
	{
		XMFLOAT3 float3 = XMFLOAT3(m_xmf3Right.x, 0.0f, m_xmf3Right.z);
		m_xmf3Right = Vector3::Normalize(float3);
		float3 = XMFLOAT3(0.0f, 1.0f, 0.0f);
		m_xmf3Up = Vector3::Normalize(float3);
		float3 = XMFLOAT3(m_xmf3Look.x, 0.0f, m_xmf3Look.z);
		m_xmf3Look = Vector3::Normalize(float3);

		m_fPitch = 0.0f;
		m_fRoll = 0.0f;

		/*Look 벡터와 월드좌표계의 z-축(0, 0, 1)이 이루는 각도(내적=cos)를 계산하여 플레이어의 y-축의 회전 각도 m_fYaw로 설정한다.*/
		float3 = XMFLOAT3(0.0f, 0.0f, 1.0f);
...
CCamera* CAirplanePlayer::ChangeCamera(DWORD nNewCameraMode, float fTimeElapsed) {
	...
	case FIRST_PERSON_CAMERA:
		...
		SetFriction(200.0f);
		XMFLOAT3 float3 = XMFLOAT3(0.0f, 0.0f, 0.0f);
		SetGravity(float3);
